...
base_package_path = Path('~/freemocap/')
print(base_package_path)
...
from pupil_freemocap_synchronizer import PupilFreemocapSynchronizer
from qt_gl_laser_skeleton_visualizer import QtGlLaserSkeletonVisualizer
from rotation_matrix_calculator import RotationMatrixCalculator
from session_data_loader import SessionDataLoader
from vor_calibrator import VorCalibrator
...
class PupilFreemocapCalibrationPipelineOrchestrator:
    ...
    def run(self):
        ...
        logger.info(f"self.raw_session_data.freemocap_timestamps.shape: {self.raw_session_data.timestamps.shape}")

        self.raw_session_data.mediapipe_skel_fr_mar_xyz = self.session_data_loader.load_mediapipe_data()
        logger.info(
        ...
        ####
        rotation_matrix_calculator = RotationMatrixCalculator(synchronized_session_data.mediapipe_skel_fr_mar_xyz)
        ...
        synchronized_session_data.right_gaze_vector_endpoint_fr_xyz = vor_calibrator.calibrate(
            pupil_labs_eye_data=copy.deepcopy(synchronized_session_data.right_eye_pupil_labs_data),
            eye_socket_rotation_data=copy.deepcopy(synchronized_session_data.right_eye_socket_rotation_data),
            head_rotation_data=copy.deepcopy(synchronized_session_data.head_rotation_data),
            fixation_point_fr_xyz=fixation_point_fr_xyz,
        )
        # left eye
...
if __name__ == "__main__":
    ...
    pupil_freemocap_calibration_pipeline_orchestrator = PupilFreemocapCalibrationPipelineOrchestrator(
        this_session_path,
